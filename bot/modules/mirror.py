...
class MirrorListener:
    ...
    def onUploadComplete(self, link: str, size, files, folders, typ, name: str):
        if not self.isPrivate and INCOMPLETE_TASK_NOTIFIER and DB_URI is not None:
            DbManger().rm_complete_task(self.message.link)
        msg = f"<b>╭── ✍️𝐅𝐈𝐋𝐄𝐍𝐀𝐌𝐄 →</b><code>{escape(name)}</code>\n\n<b>Size: </b>{size}"
        if self.isLeech:
            msg += f'\n<b>┠⌬ 📁𝐓𝐎𝐓𝐀𝐋 𝐅𝐈𝐋𝐄𝐒 →</b>{folders}'
            if typ != 0:
                msg += f'\n<b>┠⌬ 🗃️𝐂𝐎𝐑𝐑𝐔𝐏𝐓𝐄𝐃 𝐅𝐈𝐋𝐄𝐒 →</b>{typ}'
            msg += f'\n<b>╰── 🤴𝐑𝐄𝐐𝐔𝐄𝐒𝐓𝐄𝐃 𝐁𝐘 →</b>{self.tag}\n\n'
            if not files:
                sendMessage(msg, self.bot, self.message)
            else:
                fmsg = ''
            ...
            if ospath.isdir(f'{DOWNLOAD_DIR}{self.uid}/{name}'):
                msg += f'\n<b>┠⌬ 🗃️𝐒𝐔𝐁𝐅𝐎𝐋𝐃𝐄𝐑𝐒 →</b>{folders}'
                msg += f'\n<b>┠⌬ 🗂𝐅𝐈𝐋𝐄𝐒 →</b>{files}'
            msg += f'\n\n<b>╰── 🤴𝐑𝐄𝐐𝐔𝐄𝐒𝐓𝐄𝐃 →</b>{self.tag}'
            buttons = ButtonMaker()
            link = short_url(link)
            buttons.buildbutton("♻️ 𝐃𝐑𝐈𝐕𝐄 ♻️", link)
            ...
            if INDEX_URL is not None:
                ...
                if ospath.isdir(f'{DOWNLOAD_DIR}/{self.uid}/{name}'):
                    ...
                    buttons.buildbutton("🗝️ 𝐃𝐃𝐋 🗝️", share_url)
                else:
                    share_url = short_url(share_url)
                    buttons.buildbutton("🗝️ 𝐃𝐃𝐋 🗝️", share_url)
                    if VIEW_LINK:
                        ...
                        buttons.buildbutton("🎥 𝐖𝐀𝐓𝐂𝐇 🎥", share_urls)
